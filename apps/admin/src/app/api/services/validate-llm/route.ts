/**
 * GET /api/services/validate-llm
 * 
 * Proxies LLM chain validation to deploy-api.
 * The actual validation runs in deploy-api which has:
 * - HOST_AGENT_TOKEN for MLX control
 * - Direct access to all Docker services
  ...
  (async () => {
    try {
      ...
      try {
        ...
        console.warn('[API/validate-llm] Token exchange error, using session JWT:', error);
        // Continue with session JWT - deploy-api may still accept it
      }
      
      // Call deploy-api's LLM validation endpoint
      const response = await fetch(`${DEPLOY_API_URL}/api/v1/services/llm/validate`, {
        headers: {
      ...
      await writer.write(encoder.encode(sseEvent('info', 'Connected, starting LLM Validation...')));
      ...
      console.error('LLM validation proxy error:', error);
      try {
        await writer.write(encoder.encode(sseEvent('error', `Validation error: ${error.message}`, true)));
        await writer.close();
      } catch (writeError) {
