...
async function token(sessionJwt: string): Promise<string> {
  try {
    const exchange = await exchangeTokenZeroTrust(
      { sessionJwt, audience: 'deploy-api', scopes: ['admin', 'services:write'], purpose: 'vllm auto assign' },
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const response = await fetch(`${DEPLOY_API_BASE}/api/v1/services/vllm/assignments/auto`, {
      method: 'POST',
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to auto-assign', 500);
  }
}
