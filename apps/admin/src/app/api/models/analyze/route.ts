...
async function token(sessionJwt: string): Promise<string> {
  try {
    const exchange = await exchangeTokenZeroTrust(
      { sessionJwt, audience: 'deploy-api', scopes: ['admin', 'services:write'], purpose: 'models analyze' },
...
export async function POST(_request: NextRequest) {
  try {
    ...
    const response = await fetch(`${DEPLOY_API_BASE}/api/v1/services/models/analyze`, {
      method: 'POST',
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to analyze models', 500);
  }
}
