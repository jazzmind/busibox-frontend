...
async function token(sessionJwt: string): Promise<string> {
  try {
    const exchange = await exchangeTokenZeroTrust(
      { sessionJwt, audience: 'deploy-api', scopes: ['admin', 'services:read'], purpose: 'models active' },
...
export async function GET(_request: NextRequest) {
  try {
    ...
    const response = await fetch(`${DEPLOY_API_BASE}/api/v1/services/models/active`, {
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to read active models', 500);
  }
}
