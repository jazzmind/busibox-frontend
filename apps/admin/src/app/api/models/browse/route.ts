...
async function getAdminToken(sessionJwt: string): Promise<string> {
  try {
    const exchange = await exchangeTokenZeroTrust(
      { sessionJwt, audience: 'deploy-api', scopes: ['admin', 'services:read'], purpose: 'models browse' },
...
export async function GET(_request: NextRequest) {
  try {
    ...
    const token = await getAdminToken(sessionJwt);
    const response = await fetch(`${DEPLOY_API_BASE}/api/v1/services/models/browse`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!response.ok) return apiError(await response.text(), response.status);
    return apiSuccess(await response.json());
  } catch (error) {
    return apiError(error instanceof Error ? error.message : 'Failed to browse models', 500);
  }
}
