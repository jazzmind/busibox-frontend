...
import { apiError, getCurrentUserWithSessionFromCookies, requireAdmin } from '@jazzmind/busibox-app/lib/next/middleware';
...
async function token(sessionJwt: string): Promise<string> {
  try {
    const exchange = await exchangeTokenZeroTrust(
      { sessionJwt, audience: 'deploy-api', scopes: ['admin', 'services:write'], purpose: 'models download' },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${DEPLOY_API_BASE}/api/v1/services/models/download`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${await token(sessionJwt)}`,
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to download model', 500);
  }
}
