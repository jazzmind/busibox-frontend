...
async function token(sessionJwt: string): Promise<string> {
  try {
    const exchange = await exchangeTokenZeroTrust(
      { sessionJwt, audience: 'deploy-api', scopes: ['admin', 'services:write'], purpose: 'models load' },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${DEPLOY_API_BASE}/api/v1/services/models/load`, {
      method: 'POST',
      headers: {
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to load model', 500);
  }
}
