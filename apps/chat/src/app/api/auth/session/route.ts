/**
 * Consolidated auth endpoint for busibox-chat.
 *
...
const APP_NAME = process.env.APP_NAME || 'busibox-chat';
...
export async function GET(request: NextRequest) {
  try {
    // 1. Try the app-scoped session cookie (set during SSO exchange)
    ...
    // 2. Fallback: raw JWT token from cookie or Authorization header
...
// --- SSO exchange: validate token, set cookies, return user ----------------
...
// --- Token refresh: exchange SSO token for fresh authz token ---------------
